"""
Task 2: Consistent Hash Map.

Implements a circular hash map with M slots. Each physical server is
represented by K virtual servers (K = log2(M)) to spread load evenly and
minimise the "hotspot" effect when a server fails.

Hash functions (as specified in the assignment):
    H(i)     = i^2 + 2*i + 17        -> maps a REQUEST id to a slot
    Phi(i,j) = i^2 + j^2 + 2*j + 25  -> maps VIRTUAL SERVER (i, j) to a slot

    i = integer server id (assigned by the load balancer, NOT the hostname)
    j = virtual replica index for that server, 0 <= j < K

Collisions are resolved with linear probing (clockwise) to the next free
slot. Looking up a server for a request also walks clockwise from the
request's slot until it finds an occupied slot.
"""
import threading
